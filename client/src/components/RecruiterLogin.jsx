  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50">
      ...
      >
          ...
          <>
             <div className="flex flex-col items-center justify-center border-2 border-dashed border-gray-300 p-6 rounded-xl cursor-pointer hover:border-blue-400 transition">
              <label htmlFor="image" className="flex flex-col items-center gap-2 cursor-pointer">
                <img src={image ? URL.createObjectURL(image) : assets.upload_area} alt="" className="w-16 h-16 object-contain" />
        ...
        {/* Forgot Password */}
       {state === 'login' && <p className="text-sm text-blue-600 cursor-pointer hover:underline mb-4">
          Forgot Password?
        </p>}  

        {/* Submit Button */}
        <button type='submit' className="w-full bg-blue-600 text-white py-2 rounded-lg hover:bg-blue-700 transition mb-4">
        ...
        <img onClick={()=>setShowRecruiterLogin(false)} src={assets.cross_icon} alt="" className='absolute top-5 right-5 cursor-pointer' />
      </form>
    </div>
  )
}
