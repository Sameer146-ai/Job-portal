  ...
  return (
    <div className="border p-6 shadow-md rounded-2xl hover:shadow-lg transition-shadow duration-300 bg-white">
      ...
      <div className="mt-6 flex gap-4 text-sm">
        <button onClick={()=>{navigate(`/apply-job/${job._id}`); scrollTo(0,0)}} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg transition-colors duration-300">
          Apply Now
        </button>
        <button onClick={()=>{navigate(`/apply-job/${job._id}`); scrollTo(0,0)}} className="border border-gray-400 hover:border-gray-600 text-gray-600 hover:text-gray-800 px-5 py-2 rounded-lg transition-colors duration-300">
          Learn More
        </button>
      </div>
    </div>
